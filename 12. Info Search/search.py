...
def score(query:str, documents:list) -> list:
    ...
    corpus_tags = [stemmize(doc.str_tags(), stemmizer, tokenize='word') for doc in documents]

    query = stemmize(query, stemmizer, tokenize='word')
    logging.info(f'Stemmizing all docs: {((time.time() - start) * 1000):.2f}ms')
...
def retrieve(query:str) -> list:
    ...
    if len(usage) == 0:
        return documents[:50], [i for i in range(0, 50)]
    if len(usage) == 1:
        docs = zip(get_docs_by_id(usage[0]), usage[0]) 
        docs = sort_by_count(query, docs)
        return zip(*list(docs[:50]))
    ...
    docs = list(zip(get_docs_by_id(candidates[:100]), candidates[:100]))
    docs = sort_by_count(query, docs)
